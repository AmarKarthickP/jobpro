<template>
  ...
  >
    <div class="bg-white py-2 flex justify-center w-full shadow-sm sticky top-0 z-30">
      <div class="flex gap-5 md:gap-10 relative items-center w-full md:justify-center mx-5 md:mx-0 pt-5 md:pt-0">
        ...
        <!-- Task AI -->
        <router-link v-if="auth.isLoggedIn" to="/task_ai" class="hidden md:block">
          <!-- <task-a-i-button /> -->
        ...
        >
          <div :class="navClass(isActive)">
            <notification-icon class="h-7 w-7" />
          </div>
        </router-link>
        <button @click="showMenu=true" class="md:hidden">
          <menu-icon class="text-primary h-7 w-7" />
        </button>
      </div>
    </div>
  ...
  <PushMenu v-model="showMenu" class="h-0">
    <!-- Right Menu -->
    <template #menu>
      <div class="px-6 py-3 border-b">
        <h2 class="font-semibold text-2xl text-primary mt-6">Menu</h2>
      </div>
      ...
      <nav class="p-4 space-y-2">
        ...
        <button @click="openYoutube" class="w-full text-left p-3 rounded-lg hover:bg-gray-100 flex items-center gap-3">
          <youtube-icon class="h-7 w-7 text-[#ff0000]" />
...
import PushMenu from "@/components/PushMenu.vue";
